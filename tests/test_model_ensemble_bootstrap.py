...
def test_STEMClassifier_ensemble_bootstrap():
    model = make_STEMClassifier(ensemble_bootstrap=True)
...
def test_parallel_STEMClassifier_ensemble_bootstrap():
    model = make_parallel_STEMClassifier(ensemble_bootstrap=True)
    model = model.fit(X_train, np.where(y_train > 0, 1, 0))
...
def test_AdaSTEMClassifier_ensemble_bootstrap():
    model = make_AdaSTEMClassifier(ensemble_bootstrap=True)
    model = model.fit(X_train, np.where(y_train > 0, 1, 0))
